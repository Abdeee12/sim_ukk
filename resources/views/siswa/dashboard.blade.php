        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg-px-8 relative z-10">
            ...
            <div class="glass-dark rounded-2xl p-6 mb-8 border-l-4 border-cyan-500">
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">Halo, {{ $siswa->user->name }}!</h1>
                        <p class="text-slate-400">
                            {{ $siswa->kelas }} {{ $siswa->jurusan }} | NISN: <span class="font-mono text-cyan-300">{{ $siswa->nisn }}</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <a href="{{ route('siswa.createLaporan') }}" class="group block">
                ...
                <div class="glass-dark p-6 rounded-xl flex items-center space-x-4 border-l-4 border-purple-500">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-purple-400 font-bold">{{ $laporans->count() }} Hari</p>
                    </div>
                </div>
                ...
                <div class="glass-dark p-6 rounded-xl flex items-center space-x-4 border-l-4 border-yellow-500">
                    ...
                    <div>
                        ...
                        @if(optional($siswa)->mentor)
                            <p class="text-sm text-yellow-200">Ditugaskan ke: <span class="font-bold text-white">{{ $siswa->mentor->user->name }}</span></p>
                            <p class="text-xs text-slate-400">Email: <span class="font-mono text-cyan-300">{{ $siswa->mentor->user->email }}</span></p>
                        @else
                            ...
                            <tbody class="divide-y divide-slate-700">
                                @forelse($laporans as $laporan)
                                    <tr class="transition-colors hover-row">
                                        <td class="px-6 py-4 font-mono text-cyan-300">{{ \Carbon\Carbon::parse($laporan->tanggal)->format('d M Y') }}</td>
                                        <td class="px-6 py-4">{{ $laporan->jam_masuk }} - {{ $laporan->jam_keluar ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 max-w-xs overflow-hidden truncate">{{ Str::limit($laporan->detail_kegiatan, 50) }}</td>
                                        <td class="px-6 py-4 text-center">
                                            @if($laporan->path_bukti)
                                                <a href="{{ Storage::url($laporan->path_bukti) }}" target="_blank" class="text-sm text-yellow-400 hover:text-yellow-300">Lihat Bukti</a>
                                            @else
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            ...
                                            @endphp
                                            <span class="px-3 py-1 rounded-full text-xs font-bold text-white {{ $color }}">
                                                {{ ucfirst($laporan->status) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-slate-400 text-sm italic">
                                            {{ $laporan->feedback_mentor ?? 'Menunggu feedback...' }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
