    ...
    <body class="antialiased min-h-screen flex items-center justify-center relative overflow-hidden">
        ...
        <header class="absolute top-0 right-0 p-6 z-20 w-full max-w-7xl mx-auto">
            <nav class="flex justify-end gap-4">
                @if (Route::has('login'))
                    @auth
                        <a href="{{ url('/dashboard') }}" class="py-2 px-5 rounded-full font-bold text-white bg-cyan-600 hover:bg-cyan-700 transition duration-300 shadow-lg shadow-cyan-500/30">
                            Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="glass-card py-2 px-5 rounded-full font-bold text-cyan-300 hover:text-white transition duration-300 hover-neon-button border border-cyan-500/30">
                            LOGIN
                        </a>

                        {{-- Pendaftaran akun dinonaktifkan --}}
                    @endauth
                @endif
            </nav>
        </header>
        ...
        <div class="relative z-10 text-center max-w-4xl mx-auto p-4">
            ...
            <div class="flex justify-center space-x-6">
                ...
                @if (Route::has('login'))
                    @auth
                        <a href="{{ url('/dashboard') }}" class="py-3 px-8 rounded-full font-bold text-white bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 transition duration-300 hover-neon-button shadow-xl shadow-cyan-500/30">
                            Masuk ke Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="glass-card py-3 px-8 rounded-full font-bold text-cyan-300 hover:text-white transition duration-300 hover-neon-button border border-cyan-500/30">
                            LOGIN
                        </a>
