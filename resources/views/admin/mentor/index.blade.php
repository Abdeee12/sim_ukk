    ...
    <div class="pt-24 pb-12 min-h-screen bg-[#0f172a] relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 animate-fade-in-up">
                ...
                <a href="{{ route('admin.mentor.create') }}" class="mt-4 md:mt-0 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-400 hover:to-green-500 text-white font-bold py-2 px-6 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.5)] transition-all transform hover:scale-105">
                    + Tambah Mentor
                </a>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-700">
                            @forelse($mentors as $index => $mentor)
                                <tr class="transition-colors hover-neon">
                                    ...
                                    <td class="px-6 py-4 font-bold text-white">{{ $mentor->user->name }}</td>
                                    <td class="px-6 py-4 font-bold text-emerald-300">{{ $mentor->nama_perusahaan }}</td>
                                    <td class="px-6 py-4">{{ $mentor->jabatan }}</td>
                                    <td class="px-6 py-4 font-mono">{{ $mentor->no_hp }}</td>
                                    <td class="px-6 py-4 text-center flex justify-center space-x-4">
                                        
                                        <a href="{{ route('admin.mentor.edit', $mentor->id) }}" class="text-yellow-400 hover:text-yellow-300 transition transform hover:scale-110" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </a>

                                        <form action="{{ route('admin.mentor.destroy', $mentor->id) }}" method="POST" onsubmit="return confirm('Yakin hapus mentor {{ $mentor->user->name }}?');">
                                            @csrf
                                            @method('DELETE')
